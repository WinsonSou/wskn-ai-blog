...
- Spin up a small **jumphost/bastion** (Ubuntu, Rocky, RHEL, etc.) and install the key tooling: `kubectl`, `yq`, `helm`, `k9s`, `docker`, `kubeseal`, plus optional `cilium`, `flux`, and `istio` CLIs.
...
Below is a script that i always use to Download the latest airgapped bundles.
...
export REGISTRY_FQDN=registry.wskn.local/mirror

curl -Lo "nkp-air-gapped-bundle_${NKP_VERSION}_linux_amd64.tar.gz" \
# highlight-start
    "DOWNLOAD LINK URL YOU COPIED FROM THE ABOVE STEP"
# highlight-end
